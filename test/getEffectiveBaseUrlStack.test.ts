...
test("getEffectiveBaseUrlStack - multiple baseUrl fixture 1", () => {
  ...
  assert.deepEqual(baseUrlStack?.map(b => b.definedIn.fileName), [
    tsconfigPath,
    resolve(__dirname, "fixtures", "multiple-baseurl-1", "tsconfig.base.json"),
    resolve(__dirname, "fixtures", "multiple-baseurl-1", "node_modules", "@tsconfig", "docusaurus", "tsconfig.json"),
  ]);
});
...
test("getEffectiveBaseUrlStack - multiple baseUrl fixture 2", () => {
  ...
  assert.deepEqual(baseUrlStack?.map(b => b.definedIn.fileName), [
    ...
    resolve(__dirname, "fixtures", "multiple-baseurl-2", "node_modules", "@tsconfig", "docusaurus", "tsconfig.json"),
  ]);
});
